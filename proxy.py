...
def executeQueries(ip, name, query):
    with SSHTunnelForwarder(IPs[name], ssh_username="ubuntu", ssh_pkey="labsuser.pem", local_bind_address=('127.0.0.1', 3306), remote_bind_address=('127.0.0.1', 3306)) as tunnel:
        tunnel.start()
...
def direct_hit(query):
    print("Direct hit strategy! Connecting to master....")
    executeQueries('127.0.0.1', 'master', query)
